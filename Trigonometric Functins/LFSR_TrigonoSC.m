...
function [bitstream,seed_scalar] = LFSR_TrigonoSC(seed, scalar, N)
...
% taps: 8 6 5 4
if N == 256
    seed_scalar(N) = b2d(flip(seed)); %for comparison, binary to decimal conversion is performed
    ...
    for i = 1:1:255 % (2^8-1) Cycle
        ...
        seed_scalar(i) = b2d(flip(seed)); %converting for the current decimal seed
        if scalar > seed_scalar %comparator
            bitstream(i+1) = 1;
        else bitstream(i+1) = -1;
        end
...
if N == 512
    ...
    for i = 1:1:511 % (2^9-1) Cycle
        %insertion = xor(seed(9), seed(4)); %tap
        insertion = xor(xor(xor(seed(9), seed(6)), seed(4)), seed(3));
...
if N == 1024
    ...
    for i = 1:1:1023 % (2^10-1) Cycle
        insertion = xor(xor(xor(seed(10), seed(8)), seed(6)), seed(1)); %tap
...
if N == 2048
    ...
    for i = 1:1:2047 % (2^10-1) Cycle
        insertion = xor(xor(xor(seed(11), seed(5)), seed(2)), seed(1)); %tap
...
if N == 4096
    ...
    for i = 1:1:4095 
        insertion = xor(xor(xor(seed(3), seed(9)), seed(10)), seed(12));
